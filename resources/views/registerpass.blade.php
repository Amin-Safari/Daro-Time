    ...
    <div class="d-block  mt-5 vh-100 d-lg-none ">
        <div class="  ">
            <div class="m-3">
                ...
                <div class="card alert alert-succes">{{ session('succes') }}</div>
                @endif
                ...
                <form action="{{ route('register.send') }} " method="POST">   
                    @csrf
                    <div class="mb-3">
                        <label for="tel1" class="form-label">رمز جدید را وارد کنید:</label>
                        <input type="password" name="password" class="form-control" id="tel1">
                    </div>
                    <div class="mb-3">
                        <label for="tel2" class="form-label">رمز جدید را مجدد وارد کنید:</label>
                        <input type="password" name="password-confirmation" class="form-control" id="tel2">
                    </div>
                    <input type="hidden" name="token" value="{{ $token }}">
                    <div class="form-text text-danger">erore</div><br>
        ...
        <div class="rounded border border-primary" style="background-color: rgba(0, 0, 0, 0.047)">
            <div class="m-4 " style="width: 300px">
                ...
                <form action="{{ route('register.send') }} " method="POST">
                    @csrf
                    <div class="mb-3">
                        <label for="tel1a" class="form-label">رمز جدید را وارد کنید:</label>
                        <input type="password" name="password" class="form-control" id="tel1a">
                    </div>
                    <div class="mb-3">
                        <label for="tel2a" class="form-label">رمز جدید را مجدد وارد کنید:</label>
                        <input type="password" name="password_confirmation" class="form-control" id="tel2a">
                    </div>
                    <input type="hidden" name="token" value="{{ $token }}">
                    @error('password')
                    {{ $message }}
